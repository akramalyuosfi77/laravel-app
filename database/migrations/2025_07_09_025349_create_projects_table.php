...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('projects', function (Blueprint $table) {
            ...
            $table->foreignId('doctor_id')->nullable()->constrained('doctors')->after('course_id');
            ...
            $table->foreignId('course_id')->nullable()->constrained('courses')->onDelete('set null'); // المادة التي ينتمي إليها المشروع
             $table->integer('grade')->nullable()->after('semester'); // يمكن أن تكون الدرجة عددًا صحيحًا، وnullable يعني أنها ليست إجبارية في البداية
            $table->text('feedback')->nullable()->after('grade'); // يمكن أن تكون الملاحظات نصًا، وnullable
            // سيتم إضافته بعد عمود doctor_id ليكون الكود منظماً
            $table->enum('supervision_status', ['pending', 'approved', 'rejected'])->nullable()->after('doctor_id'); // مهم لكي لا يسبب مشاكل مع المشاريع التي ليس لها مشرف
