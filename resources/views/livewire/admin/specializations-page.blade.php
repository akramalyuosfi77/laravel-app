<div x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
            ...
            <div class="grid md:grid-cols-2 gap-8 items-center">
                ...
                <div class="order-1 md:order-1 flex justify-center">
                    <div class="relative">
                        ...
                        <lottie-player
                            src="{{ asset('animations/Demo.json') }}"
                ...
                <div class="order-2 md:order-2">
                    ...
                    <p class="text-lg text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
                        بناء المسارات التعليمية للطلاب بكل احترافية وإبداع
                    </p>
    ...
    <div wire:loading.class.delay="opacity-50" class="transition-opacity">
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                ...
                <div class="group relative bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm p-6 rounded-2xl border {{ $theme['border'] }} transition-all duration-300 hover:shadow-2xl hover:shadow-{{ explode('-', $theme['gradient'])[1] }}-500/20 hover:-translate-y-2 overflow-hidden">
                    ...
                    <div class="relative z-10 flex flex-col h-full">
                        ...
                        <div class="flex items-start gap-4 mb-5">
                            ...
                            <div class="flex-1">
                                <span class="inline-block px-3 py-1 {{ $theme['badge'] }} rounded-full text-xs font-bold mb-2">تخصص أكاديمي</span>
                                <h3 class="font-bold text-xl text-zinc-900 dark:text-white leading-tight group-hover:{{ $theme['text'] }} transition-colors">{{ $spec->name }}</h3>
                            </div>
                        </div>
                        ...
                        <div class="flex-grow space-y-3 mb-5">
                            <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl">
                                ...
                                <span class="text-sm text-zinc-800 dark:text-zinc-200 font-bold">{{ $spec->department?->name ?? 'غير محدد' }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl">
                                <span class="text-xs font-semibold text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $theme['bg'] }}"></span>
                                ...
                                <span class="text-sm text-zinc-800 dark:text-zinc-200 font-bold">{{ $spec->duration }}</span>
                            </div>
                        </div>

                        {{-- Info Footer --}}
                        <div class="pb-4 mb-4 border-b border-zinc-200 dark:border-zinc-700/50">
                            <span class="text-xs text-zinc-400 dark:text-zinc-500">تم الإنشاء: {{ $spec->created_at->format('Y/m/d') }}</span>
                        </div>

                        {{-- Action Buttons --}}
                        <div class="grid grid-cols-2 gap-2">
                            <button wire:click="edit({{ $spec->id }})" class="flex flex-col items-center gap-1.5 p-3 bg-slate-600 hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-xl text-white transition-all hover:scale-105 shadow-md hover:shadow-lg">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                <span class="text-xs font-bold">تعديل</span>
                            </button>
                            <button wire:click="confirmDelete({{ $spec->id }})" class="flex flex-col items-center gap-1.5 p-3 bg-red-500 hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700 rounded-xl text-white transition-all hover:scale-105 shadow-md hover:shadow-lg">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            @empty
                <div class="col-span-1 md:col-span-2 xl:col-span-3 2xl:col-span-4 p-12 text-center bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-zinc-800/50 dark:to-zinc-900/50 rounded-3xl border-2 border-dashed border-blue-300 dark:border-blue-700/50 backdrop-blur-sm">
                    ...
                    <h3 class="text-2xl font-bold text-zinc-800 dark:text-white mb-2">ارسم ملامح المستقبل</h3>
                    <p class="text-zinc-600 dark:text-zinc-400 mb-6 max-w-md mx-auto">لا توجد تخصصات بعد. ابدأ بإضافة تخصص جديد لوضع حجر الأساس لمسارات تعليمية متميزة.</p>
    ...
    <div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" x-data x-transition.opacity>
        <div @click.away="window.livewire.dispatch('close-form')" class="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col border border-zinc-200 dark:border-zinc-700">
            ...
            <div class="flex-shrink-0 p-5 flex justify-between items-center border-b border-zinc-200 dark:border-zinc-700">
                <h2 class="text-lg font-bold text-zinc-900 dark:text-white">{{ $edit_id ? 'تعديل التخصص' : 'إضافة تخصص جديد' }}</h2>
            ...
            <form wire:submit.prevent="save" class="flex-grow p-6 space-y-5 overflow-y-auto">
                <div>
                    <label for="name" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">اسم التخصص <span class="text-red-500">*</span></label>
                    <input id="name" wire:model="name" type="text" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" placeholder="مثال: هندسة البرمجيات">
                ...
                <div>
                    ...
                    <textarea id="description" wire:model="description" rows="3" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-y transition-all" placeholder="وصف موجز للتخصص..."></textarea>
                    @error('description') <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="duration" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">مدة التخصص <span class="text-red-500">*</span></label>
                    <input id="duration" wire:model="duration" type="text" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" placeholder="مثال: 4 سنوات">
                    @error('duration') <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="department_id" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">القسم التابع له <span class="text-red-500">*</span></label>
                    <select id="department_id" wire:model="department_id" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
            ...
            <div class="flex-shrink-0 p-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-200 dark:border-zinc-700">
                ...
                <button type="submit" wire:click="save" class="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors shadow-md shadow-indigo-500/20">
                    <span wire:loading.remove wire:target="save">{{ $edit_id ? 'حفظ التعديلات' : 'إنشاء التخصص' }}</span>
    ...
    <div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" x-data x-transition.opacity>
        <div @click.away="$wire.set('delete_id', null)" class="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-zinc-200 dark:border-zinc-700">
            ...
            <div class="p-6 text-center">
                ...
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">هل أنت متأكد من حذف هذا التخصص؟ لا يمكن التراجع عن هذا الإجراء.</p>
            </div>
