<div>
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-800 dark:text-white">إدارة الدفعات</h1>
            <p class="mt-1 text-zinc-500 dark:text-zinc-400">تنظيم، متابعة، وتحكم كامل في دفعات الطلاب.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div class="relative">
            ...
            <input wire:model.live.debounce.300ms="search" placeholder="ابحث بالاسم, السنة, التخصص..." class="w-full pr-11 pl-4 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-xl bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 transition-all">
        </div>
        {{-- فلتر القسم --}}
        <select wire:model.live="filter_department_id" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-xl bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 transition-all">
        ...
        {{-- فلتر التخصص --}}
        <select wire:model.live="filter_specialization_id" class="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-xl bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 transition-all">
                ...
                <div class="group relative bg-white dark:bg-zinc-800/50 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-700 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5">
                    ...
                    <div class="flex flex-col h-full">
                        <div class="flex-grow">
                            ...
                            <div class="space-y-3 text-sm">
                                <div class="flex justify-between">
                                    <span class="font-semibold text-zinc-500 dark:text-zinc-400">التخصص:</span>
                                    <span class="text-zinc-700 dark:text-zinc-200 font-medium">{{ $batch->specialization->name ?? 'غير محدد' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="font-semibold text-zinc-500 dark:text-zinc-400">القسم:</span>
                                    <span class="text-zinc-700 dark:text-zinc-200 font-medium">{{ $batch->specialization->department->name ?? 'غير محدد' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="font-semibold text-zinc-500 dark:text-zinc-400">سنة البدء:</span>
                                    <span class="text-zinc-700 dark:text-zinc-200 font-medium">{{ $batch->start_year }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-700 flex justify-between items-center text-xs text-zinc-500 dark:text-zinc-400">
                            <span>المستوى: السنة {{ $batch->academic_year }} - الترم {{ $batch->semester }}</span>
                            <span>{{ $batch->created_at->format('Y/m/d' ) }}</span>
                        </div>
                    </div>

                    <div class="absolute inset-0 {{ $theme['overlay'] }} dark:bg-zinc-900/80 backdrop-blur-sm rounded-2xl flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button wire:click="viewBatch({{ $batch->id }})" class="w-12 h-12 flex items-center justify-center bg-white/20 hover:bg-white/30 rounded-full text-white transform transition-all hover:scale-110 shadow-lg" title="عرض التفاصيل">
                            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        </button>
                        <button wire:click="edit({{ $batch->id }} )" class="w-12 h-12 flex items-center justify-center bg-white/20 hover:bg-white/30 rounded-full text-white transform transition-all hover:scale-110 shadow-lg" title="تعديل">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </button>
                        <button wire:click="confirmDelete({{ $batch->id }})" class="w-12 h-12 flex items-center justify-center bg-red-500/30 hover:bg-red-500/40 rounded-full text-white transform transition-all hover:scale-110 shadow-lg" title="حذف">
  ...
  {{-- View Details Modal (Corrected and Complete) --}}
@if($showViewModal)
<div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" x-data x-transition.opacity>
